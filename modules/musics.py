...
from modules.config import proxies, rank_query_ban_servers, env
...
masterdatadir = os.path.join(botpath, 'masterdata', 'realtime')


def idtoname(musicid):
    with open(os.path.join(masterdatadir, 'musics.json'), 'r', encoding='utf-8') as f:
        musics = json.load(f)
    for i in musics:
        if i['id'] == musicid:
            return i['title']
...
def levelRankPic(level, difficulty, fcap=0, userid=None, isprivate=False, server='jp', qqnum='未知'):
    ...
    title = f'{difficulty.upper()} {level if level != 0 else ""} 难度表'
    playLevelKey = "playLevel"
    ...
    if userid is not None and not isprivate:
        ...
        try:
            ...
            rankPic = singleLevelRankPic(musicData, difficulty, profile.musicResult, oneRowCount=10 if level != 0 else 8)
        except:
            profile.isNewData = True
            rankPic = singleLevelRankPic(musicData, difficulty, oneRowCount=10 if level != 0 else 8)
            error = True
    else:
        rankPic = singleLevelRankPic(musicData, difficulty, oneRowCount=10 if level != 0 else 8)
...
def notecount(count):
    text = ''
    with open(os.path.join(masterdatadir, 'musicDifficulties.json'), 'r', encoding='utf-8') as f:
        data = json.load(f)
    for i in data:
        try:
