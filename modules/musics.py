...
def levelRankPic(level, difficulty, fcap=0, userid=None, isprivate=False, server='jp', qqnum='未知'):
    ...
    if fcap == 0:
        title = f'{difficulty.upper()} {level if level != 0 else ""} 难度表（不分FCAP，仅供参考）'
    ...
    font_style = ImageFont.truetype("fonts/SourceHanSansCN-Bold.otf", 22)
    updatetime = time.localtime(os.path.getmtime(r"masterdata/realtime/musicDifficulties.json"))
    draw.text((50, pic.size[1] - 110), '※小数定数来自Twitter:@MaengZombie 仅供参考', fill='#00CCBB', font=font_style)
